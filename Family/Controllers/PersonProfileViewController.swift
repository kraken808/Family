...
class PersonProfileViewController: UIViewController {
    ...
    let imageView = UIImageView(image: #imageLiteral(resourceName: "fati"), contentMode: .scaleAspectFill)
    ...
    init(user: MUser) {
        ...
        self.aboutMeLabel.text = "default description"
        self.imageView.sd_setImage(with: URL(string: user.avatarURL), completed: nil)
        self.chat = MChat(friendUsername: user.name,
    ...
    @objc private func sendMessage() {
        ...
        self.chat.lastMessageContent = message
        self.dismiss(animated: true) {
            FirestoreService.shared.createActiveChat(user: self.user, message: message){
                (result) in
                switch result {
...
extension PersonProfileViewController {
